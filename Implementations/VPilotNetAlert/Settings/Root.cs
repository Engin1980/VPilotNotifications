...
namespace VPilotNetAlert.Settings
{
  public class Root
  {
    ...
    internal static Root Load()
    {
      ...
      Root? ret = JsonConvert.DeserializeObject<VPilotNetAlert.Settings.Root>(s);
      EAssert.IsNotNull(ret, "Settings file is empty or invalid. Please check the file: " + configFileName);

      if (isPluginMode) ret.AdjustPaths();
      ret.Validate(Logger.Create(typeof(Root)));
      return ret;
    }
    ...
    private void Validate(Logger logger)
    {
      ...
      if (this.Vatsim == null)
        Fail("Vatsim is undefined.");
      else
      {
        if (this.Vatsim.Sources == null)
          Fail("Vatsim sources are undefined.");
        else
